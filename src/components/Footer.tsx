  ...
  return (
    <footer className="bg-alt pt-20 pb-10 px-6 lg:px-12 border-t border-border">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-12 mb-16">
        ...
        <div>
          <h4 className="text-main font-semibold tracking-wide mb-6 uppercase text-sm">Navegación</h4>
          <ul className="flex flex-col gap-3">
            {['Inicio', 'Construcción', 'Inmobiliaria', 'Terrenos', 'Casas', 'Nosotros', 'Contacto'].map(item => (
              <li key={item}>
                <a href={`#${item.toLowerCase()}`} className="text-muted hover:text-brand transition-colors font-medium">
                  {item}
                </a>
              </li>
            ))}
        ...
        <div>
          <h4 className="text-main font-semibold tracking-wide mb-6 uppercase text-sm">Contacto</h4>
          <ul className="flex flex-col gap-3 text-muted font-medium">
            <li>México</li>
            <li>
